...
def add_diff_columns(table_new, table_ref, value_index_in_row, bigrams, is_integer_value):
    table_with_diff_columns              = []
    table_with_diff_removed_columns      = []
    symbol_entry_and_pos_lookup_dict     = {}
    symbol_ref_entry_and_pos_lookup_dict = {}
    key_index                            = 0

    # Create lookup dictionary for the new table
    for row_index, row_data in enumerate(table_new):
        if row_index == 0:
            key_index = row_data.index('Bigram') if bigrams else row_data.index('Symbol')
        else:
            symbol_entry_and_pos_lookup_dict[row_data[key_index]] = [row_index, row_data[value_index_in_row]]  # Entry in table, num appearances

    # Create lookup dictionary for the reference table
    for row_index, row_data in enumerate(table_ref):
        if row_index == 0:
            key_index = row_data.index('Bigram') if bigrams else row_data.index('Symbol')
        else:
            symbol_ref_entry_and_pos_lookup_dict[eval(row_data[key_index]) if bigrams else row_data[key_index]] = [row_index, row_data[value_index_in_row]]  # Entry in table, num appearances
    ...
    for row_index, row_data in enumerate(table_ref):
        ...
        if row_index == 0:  # title
            ...
            key_index = new_row.index('Bigram') if bigrams else new_row.index('Symbol')
            table_with_diff_removed_columns.append(new_row)
        else:
            # current_symbol = str(row_data[key_index]) if bigrams else row_data[key_index]
            current_symbol = eval(row_data[key_index]) if bigrams else row_data[key_index]
            if current_symbol in symbol_entry_and_pos_lookup_dict:
                diff_entries = symbol_entry_and_pos_lookup_dict[current_symbol][0] - row_index
    ...
    return table_with_diff_columns, table_with_diff_removed_columns


# Some entries have 'New' In their sorted column of entry, so save those aside to a dedicated file
def sort_and_save_stats_no_lookup(stats_filename, stats, sort_by_col, reverse, place_strs_on_sorted_top=True):
    ...
    for index,row in enumerate(stats_to_process):
        if isinstance(row[sort_by_col], str) and 'New' in row[sort_by_col]:
            new_rows.append(row)
        else:
            rows_to_sort.append(row)

    [rows_to_sort, str_rows_to_sort] = pdf_generator.remove_str_rows(rows_to_sort, sort_by_col)
    sorted_rows = sorted(rows_to_sort, key=lambda row: row[sort_by_col], reverse=reverse)
    if place_strs_on_sorted_top:
        sorted_rows = str_rows_to_sort + sorted_rows
    else:
        sorted_rows = sorted_rows + str_rows_to_sort
...
def post_process_etfs(csv_db_path, date_time_path, csv_db_filename):
    ...
    [diff_num_appearances_table,         diff_num_appearances_table_ref        ] = add_diff_columns(table_new=num_appearances_table,         table_ref=num_appearances_table_ref,         value_index_in_row=ReportTableColumns.VALUE.value, bigrams=False, is_integer_value=True )
    [diff_sum_weights_table,             diff_sum_weights_table_ref            ] = add_diff_columns(table_new=sum_weights_table,             table_ref=sum_weights_table_ref,             value_index_in_row=ReportTableColumns.VALUE.value, bigrams=False, is_integer_value=False)
    [diff_bigrams_num_appearances_table, diff_bigrams_num_appearances_table_ref] = add_diff_columns(table_new=num_bigrams_appearances_table, table_ref=num_bigrams_appearances_table_ref, value_index_in_row=ReportTableColumns.VALUE.value, bigrams=True,  is_integer_value=True )
    [diff_bigrams_sum_weights_table,     diff_bigrams_sum_weights_table_ref    ] = add_diff_columns(table_new=sum_bigrams_weights_table,     table_ref=sum_bigrams_weights_table_ref,     value_index_in_row=ReportTableColumns.VALUE.value, bigrams=True,  is_integer_value=False)
    ...
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=diff_num_appearances_table_ref,                       post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Bottom_Dis-Appearances',              reported_column_index=ReportTableColumns.VALUE.value,        reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=True )
    print('Generating diff_sum_weights_tables.\n')
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=diff_sum_weights_table,                               post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Leading_Weight',                      reported_column_index=ReportTableColumns.VALUE.value,        reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=False)
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=diff_sum_weights_table_ref,                           post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Leading_Weight-Dis-Appearances',      reported_column_index=ReportTableColumns.VALUE.value,        reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=False)
    print('Generating diff_bigrams_num_appearances_tables.\n')
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=diff_bigrams_num_appearances_table,                   post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_Appearances',                 reported_column_index=ReportTableColumns.VALUE.value,        reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False )
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=diff_bigrams_num_appearances_table,                   post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_Appearances_Diff+New',        reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False, sort_csv_rows=True, place_strs_on_sorted_top=True,  reverse_sort=True )
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=diff_bigrams_num_appearances_table,                   post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_Appearances_Diff+',           reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False, sort_csv_rows=True, place_strs_on_sorted_top=False, reverse_sort=True )
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=diff_bigrams_num_appearances_table,                   post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_Appearances_Diff-',           reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False, sort_csv_rows=True, place_strs_on_sorted_top=False, reverse_sort=False)
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=diff_bigrams_num_appearances_table_ref,               post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_Dis-Appearances',             reported_column_index=ReportTableColumns.VALUE.value,        reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False )
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=diff_bigrams_num_appearances_table_ref,               post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Bottom_Dis-Appearances',              reported_column_index=ReportTableColumns.VALUE.value,        reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=True  )
    print('Generating diff_bigrams_sum_weights_tables.\n')
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=diff_bigrams_sum_weights_table,                       post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_Weight',                      reported_column_index=ReportTableColumns.VALUE.value,        reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False )
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=diff_bigrams_sum_weights_table_ref,                   post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_Weight-Dis-Appearances',      reported_column_index=ReportTableColumns.VALUE.value,        reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False )
    print('Generating most_increased_appearancs_values_tables.\n')
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_appearancs_values_table,               post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Appearances_Values_Diff+New',         reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=False, sort_csv_rows=True, place_strs_on_sorted_top=True,  reverse_sort=True )
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_appearancs_values_table,               post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Appearances_Values_Diff+',            reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=False, sort_csv_rows=True, place_strs_on_sorted_top=False, reverse_sort=True )
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_appearancs_values_table,               post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Appearances_Values_Diff-',            reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=True )
    print('Generating new_most_increased_appearancs_values_tables.\n')
    pdf_generator.csv_to_pdf(                report_table=new_most_increased_appearancs_values_table,           post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Leading_New_Appearances_Values_Diff', reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=False)
    pdf_generator.csv_to_pdf(                report_table=new_most_increased_appearancs_values_table,           post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Bottom_New_Appearances_Values_Diff',  reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=True )
    print('Generating most_increased_appearances_entries_tables.\n')
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_appearances_entries_table,             post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Leading_Appearances_Entries_Diff',    reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=False)
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_appearances_entries_table,             post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Bottom_Appearances_Entries_Diff',     reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=True )
    print('Generating new_most_increased_appearances_entries_tables.\n')
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=new_most_increased_appearances_entries_table,         post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Leading_New_Appearances_Entries_Diff',reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=False)
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=new_most_increased_appearances_entries_table,         post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Bottom_New_Appearances_Entries_Diff', reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=True )
    print('Generating most_increased_weights_values_tables.\n')
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_weights_values_table,                  post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Leading_Weight_Values_Diff',          reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=False)
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_weights_values_table,                  post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Bottom_Weight_Values_Diff',           reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=True )
    print('Generating new_most_increased_weights_values_tables.\n')
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=new_most_increased_weights_values_table,              post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Leading_New_Weight_Values_Diff',      reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=False)
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=new_most_increased_weights_values_table,              post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Bottom_New_Weight_Values_Diff',       reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=True )
    print('Generating most_increased_weights_entries_tables.\n')                                                                                                                                                                                                  
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_weights_entries_table,                 post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Leading_Weight_Entries_Diff',         reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=False)
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_weights_entries_table,                 post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Bottom_Weight_Entries_Diff',          reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=True )
    print('Generating new_most_increased_weights_entries_tables.\n')                                                                                                                                                                                              
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=new_most_increased_weights_entries_table,             post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Leading_New_Weight_Entries_Diff',     reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=False)
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=new_most_increased_weights_entries_table,             post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_ENTRIES,        report_title='Bottom_New_Weight_Entries_Diff',      reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=False, reverse=True )
    print('Generating most_increased_bigrams_appearancs_values_tables.\n')                                                                                                                                                                                        
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_bigrams_appearancs_values_table,       post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_Appearances_Values_Diff',     reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False)
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_bigrams_appearancs_values_table,       post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Bottom_Appearances_Values_Diff',      reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=True )
    print('Generating new_most_increased_bigrams_appearancs_values_tables.\n')                                                                                                                                                                                    
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=new_most_increased_bigrams_appearancs_values_table,   post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_New_Appearances_Values_Diff', reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False)
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=new_most_increased_bigrams_appearancs_values_table,   post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Bottom_New_Appearances_Values_Diff',  reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=True )
    print('Generating most_increased_bigrams_appearances_entries_tables.\n')                                                                                                                                                                                      
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_bigrams_appearances_entries_table,     post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_Appearances_Entries_Diff',    reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False)
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_bigrams_appearances_entries_table,     post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Bottom_Appearances_Entries_Diff',     reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=True )
    print('Generating new_most_increased_bigrams_appearances_entries_tables.\n')                                                                                                                                                                                  
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=new_most_increased_bigrams_appearances_entries_table, post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_New_Appearances_Entries_Diff',reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False)
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=new_most_increased_bigrams_appearances_entries_table, post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Bottom_New_Appearances_Entries_Diff', reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='#',      append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=True )
    print('Generating most_increased_bigrams_weights_values_tables.\n')                                                                                                                                                                                           
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_bigrams_weights_values_table,          post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_Weight_Values_Diff',          reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False)
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=most_increased_bigrams_weights_values_table,          post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Bottom_Weight_Values_Diff',           reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=True )
    print('Generating new_most_increased_bigrams_weights_values_tables.\n')                                                                                                                                                                                       
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=new_most_increased_bigrams_weights_values_table,      post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_New_Weight_Values_Diff',      reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False)
    pdf_to_append = pdf_generator.csv_to_pdf(report_table=new_most_increased_bigrams_weights_values_table,      post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Bottom_New_Weight_Values_Diff',       reported_column_index=ReportTableColumns.DIFF_VALUE.value,   reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=True )
    print('Generating most_increased_bigrams_weights_entries_tables.\n')                                                                                                                                                                                          
    pdf_generator.csv_to_pdf(                report_table=most_increased_bigrams_weights_entries_table,         post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_Weight_Entries_Diff',         reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False)
    pdf_generator.csv_to_pdf(                report_table=most_increased_bigrams_weights_entries_table,         post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Bottom_Weight_Entries_Diff',          reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=True )
    print('Generating new_most_increased_bigrams_weights_entries_tables.\n')                                                                                                                                                                                      
    pdf_generator.csv_to_pdf(                report_table=new_most_increased_bigrams_weights_entries_table,     post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Leading_New_Weight_Entries_Diff',     reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='Weight', append_to_pdf=pdf_to_append, output=False, bigrams=True,  reverse=False)
    pdf_generator.csv_to_pdf(                report_table=new_most_increased_bigrams_weights_entries_table,     post_process_path_new=csv_db_path+date_time_path, limit_num_rows=NUM_REPORTED_BIGRAM_ENTRIES, report_title='Bottom_New_Weight_Entries_Diff',      reported_column_index=ReportTableColumns.DIFF_ENTRIES.value, reported_column_name='Weight', append_to_pdf=pdf_to_append, output=True , bigrams=True,  reverse=True )
